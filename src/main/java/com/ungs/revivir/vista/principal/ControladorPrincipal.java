...
public class ControladorPrincipal implements ClienteInvocable, ServicioInvocable, UsuarioInvocable,
	...
	public void actualizarFallecidos(Fallecido nuevo) {
		if (controladorInterno instanceof FallecidoInvocable)
			((FallecidoInvocable)controladorInterno).actualizarFallecidos(nuevo);
	}	
				...
				if (selecRestauraBack == 1) {
					...
					try {
						...
						Process p = Runtime.getRuntime().exec(
								"C:\\Program Files\\MySQL\\MySQL Server 5.5\\bin\\mysql -uroot -proot revivir");
		...
		if (resp == JFileChooser.APPROVE_OPTION) {
			try {
				...
				Process child = runtime.exec(
						"C:\\Program Files\\MySQL\\MySQL Server 5.5\\bin\\mysqldump --opt --password=root --user=root --databases revivir");
